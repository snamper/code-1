"use strict";
import { createdActiveFn } from '@/services/container/platform/createdActive';
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/platform/addGoodsPack') {
  ...
  effects:{
    *query({payload = {},pageNo,pageSize},{put,call,select}){  //获取列表
      ...
      const data = yield call(createdActiveFn, payload);
      if(data.success){
