...
const FilterItem = ({
  ...
  const getSelectName = (children) => {  //联动搜索
    selectNames = children;
    let initialSection = []
    if(query.selectRange){ //如果当前搜索条件中有当前时间名称
     	if(typeof( query.selectRange) === 'object'){
     		for(let i = 0; i < query.selectRange.length; i++){
	      	initialSection.push(Number(query.selectRange[i]))
	      }
     	}else initialSection.push(Number(query.selectRange))
    }
  ...
  const handleFields = (values) => {  //时间格式转化
    ...
    if (selectNames && selectNames.length > 0) {
			values[selectNames[0]] = values.selectRange[values.selectRange.length - 1]
			if(values.selectRange.length > 1) values[selectNames[1]] = 2
			else if(values.selectRange.length > 0) {
				values[selectNames[1]] = 1
			}
    }
    return values
  }
  ...
  return (
    <Form layout="inline" style={{marginBottom:8}} >
      {filterArray.length > 0?filterArray.map(item => (
          ...
          item.type === 'select2' ?
          <FormItem label={item.label} key={item.label}>
            {getFieldDecorator(item.name, { initialValue: getSelectName(item.children)})(
              <Cascader filedNames={item.filedNames} options={item.options} placeholder="请选择" />
            )}
          </FormItem>: ''
        )) :''
      }
