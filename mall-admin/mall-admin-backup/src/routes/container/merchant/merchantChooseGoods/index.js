...
const channelChooseGoods = ({goodsCate,loading,dispatch,location})=>{
  const columns = [{
    ...
    render: (text, record) => (
        ...
        <span >
          <a onClick={() => {dispatch({type:'goodsCate/chooseGoods',payload:{sortId:record.id,pageNo:goodsCate.currentPageModal,pageSize:goodsCate.pageSizeModal,channelId:goodsCate.channelId}})}}>商品选取</a>
        </span> : ''
    ),
  }];
  const rowSelection = {
