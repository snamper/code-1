  ...
  componentWillMount(){
    ...
    DeviceEventEmitter.addListener("updateUi",function(){
      ...
      for(let i=0;i<robotPlayers.length;i++){
        if(robotPlayers[i].seatNo == Number(player.seatNo)+4||Number(robotPlayers[i].seatNo)+5 == player.seatNo){
          _this.setState({        
            display:"flex",
          ...
          _this.bigBlindEnd(robotPlayers[i].seatNo);
        ...
        _this.timer = setTimeout(()=>{
          ...
          if(randomActive>0.95){  //raise
            if(Number(_this.state.stake)<Number(store.getState().table.data.maxBlind)){
              const stake = Number(_this.state.stake)
              _this.setState({
              ...
              // store.getState().table.data.pots[0].allIn.push( stake )
          ...
          }else if(randomActive>0.01){ //call
            const chipValue = _this.state.chipValue;
            _this.setState({
          ...
          _this.timer = setTimeout(()=>{
            ...
            if(randomActive>0.95){  //raise
              ...
              if(Number(_this.state.stake)<Number(store.getState().table.data.maxBlind)){
                ...
                // store.getState().table.data.pots[0].allIn.push( Number(stake)+Number(chipValue) )
                //store.getState().table.data.pots[0].allInPlayer.push( seatNo ) // 记录all in 玩家座位号
                store.getState().table.data.pots[1].allInData.push({ "allIn":Number(chipValue)+Number(stake),"seatNo": seatNo }) //记录all in 玩家筹码及座位号
                if(store.getState().table.data.pots[0].allPot.length && store.getState().table.data.pots[0].allPot.length>0){
                  const allPotLength = store.getState().table.data.pots[0].allPot.length - 1
  ...
  render() {
    return (
      <View style={styles.wrap}>
        ...
        <Chip wrap="big" text={this.state.chipValue} align="leftLeftTop" style={{display:this.state.chipDisplay}}/>
        <RunChip wrap="big" change = {() =>{this.runChipChange()}} text={this.state.chipValue} location={{x:110,y:-5}} style={{display:this.state.runChipDisplay}} align="leftLeftTop"/>
