        ...
        _this.timer = setTimeout(()=>{
          ...
          if(randomActive>0.95){  //raise
            if(Number(_this.state.stake)<Number(store.getState().table.data.maxBlind)){
              ...
              // store.getState().table.data.pots[0].allIn.push( stake + chipValue )
  ...
  render() {
    return (
      <View style={styles.wrap}>
        ...
        <RunChip wrap="big" text={this.state.chipValue} location={{x:-205,y:-10}} style={{display:this.state.runChipDisplay}} change = {() =>{this.runChipChange()}} align="rightRightCenter"/>
