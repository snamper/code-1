<template>
  ...
  <div class="phoneInput pwdInput">
        ...
        <input type="password" maxlength='16' v-on:input="pwdChange" minLength='6' name="tel"  v-model="password" placeholder="请输入密码（6-16位数字，字母组合）" value="" />
    </div>
  <div class="phoneInput pwdInput">
        <span class="icon">确认密码</span>
        <input type="password" maxlength='16' v-on:input="pwdChange" minLength='6' name="tel"  v-model="password1" placeholder="请输入密码（6-16位数字，字母组合）" value="" />
    </div>
...
export default {
  ...
  methods: {
    pwdChange() {
      if(  this.password.length > 0 && this.password1.length > 0 && this.password1 == this.password  ){
    ...
    goConfim(){
      ...
      that.$store.dispatch('commonService',{"apid": 70,params: {
              signature:that.check,
              loginPwd:that.password,
              confirmLoginPwd:that.password1,
              userId:that.userId,
              phone:that.phone
          }})
            .then((result) => {
