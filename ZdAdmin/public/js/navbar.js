...
layui.define(['element', 'common'], function(exports) {
	...
	function getHtml(data) {
		...
		var secResource = data[0].children;
		
		for(var i = 0; i < secResource.length; i++) {
			var flagItem = true;
			if(secResource[i].children !== undefined && secResource[i].children.length > 0) {
				for(var j = 0; j < secResource[i].children.length; j++) {
					if($(".navContent").attr("data-navThird") == secResource[i].children[j].href){
						ulHtml += '<li class="layui-nav-item layui-nav-itemed">';
						flagItem = false;
					}
				}
			}
			if($(".navContent").attr("data-nav") == secResource[i].href && !secResource[i].children){
				if(flagItem){
			...
			if(secResource[i].children !== undefined && secResource[i].children.length > 0) {
				ulHtml += '<a href="javascript:;">' + secResource[i].title;
				ulHtml += '</a>';
				ulHtml += '<dl class="layui-nav-child" >'
				for(var j = 0; j < secResource[i].children.length; j++) {
					if($(".navContent").attr("data-navThird") == secResource[i].children[j].href){
						ulHtml += '<dd class="layui-this" style="padding-left:15px" title="'+secResource[i].children[j].title+'">';
					}else{
						ulHtml += '<dd style="padding-left:15px" title="'+secResource[i].children[j].title+'">';
					}
					
					ulHtml += '<a href='+secResource[i].children[j].href +'>' +secResource[i].children[j].title ;
					ulHtml += '</a>';
					ulHtml += '</dd>';
				}
				ulHtml += '</dl>';
				...
				ulHtml += '<a href='+secResource[i].href+'>' + secResource[i].title;
				ulHtml += '</a>';
			}
			ulHtml += '</li>';
		}
