...
layui.use(['element', 'laydate', 'upload', 'form' ], function(){
	...
	if($(".status").attr("data-status")) status = $(".status").attr("data-status");
...
//		return
		$.ajax({
			...
			url: "/admin/business/platform/swap/cbusinessplatform.do",
	...
	//校验
	$(".pointName").on("blur", function() {
		if(!$('.pointName').val() || !$('.pointName').check().number(1,10)){
			$(".tooltip-pointName").show()
		}else
			$(".tooltip-pointName").hide()
	})
	$(".merchantCode").on("blur", function() {
		if(!$('.merchantCode').val()){
			$(".tooltip-merchantCode").show()
		}else
			$(".tooltip-merchantCode").hide()
	})
