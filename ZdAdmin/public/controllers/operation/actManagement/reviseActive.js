...
layui.use(['element', 'laydate', 'upload', 'form', 'paging'], function(){
	...
	var nameFlag = true, intFlag = true, limitFlag = true;
	...
	$(".saveAddActive").on("click", function() {
		if(nameFlag && intFlag && limitFlag) {
			...
			var data = {
				id: getQueryString("spreadEventId"),
				name: name,
			...
			btn: ['确认', '取消'] //按钮
					...
					$.ajax({
						type: "post",
						dataType: "json",
						url: "/admin/spread/event/update.do",
						data:data,
	...
	if( getQueryString("id") ) {
		...
		$.ajax({
				type: "get",
				dataType: "json",
				url: "/admin/spread/channel/getChannelDetail.do",
				data: {
				...
				success: function(json) {
					if(json.message == "成功") {
						moudle = json.data.fundname;
						bk= json.data.bank;
						var bankList = ['中国银行','北京银行','中国工商银行','招商银行','中国农业银行','中国建设银行','中国邮政储蓄银行','交通银行','中国民生银行','浦发银行']
						var htmlBank = '<option value="">请选择银行</option><option value=" ">请选择银行</option>'
						for(var i = 0; i < bankList.length; i++){
							if (  bk == bankList[i] ) {
								htmlBank += '<option selected value='+ bankList[i] +'>'+ bankList[i] +'</option>'
							}else{
							 	htmlBank += '<option value='+ bankList[i]  +'>'+ bankList[i]  +'</option>'
							}
						}
						$("#bankSelect").empty().html(htmlBank);
							...
							success: function(json) {
								if(json.message == "成功") {
									...
									for(var i = 0; i < json.data.datas.list.length; i++){
										if (  moudle == json.data.datas.list[i].name ) {
											html += '<option selected value='+ json.data.datas.list[i].id +'>'+ json.data.datas.list[i].name +'</option>'
										}else{
	...
	//渠道验证
	var channelFlag = true, contactsFlag = true, phonelFlag = true, detailFlag = true, bankFlag = true, accountslFlag = true,remarksFlag = true;	
		...
		if(  channelFlag && contactsFlag && phonelFlag && detailFlag && bankFlag && accountslFlag && remarksFlag ) {
			...
			layer.confirm('保存后及时生效，是否保存？', {
				btn: ['确认', '取消'] //按钮
						...
						$.ajax({
							type: "post",
							url: "/admin/spread/channel/updatechannel.do",
							data:data,
							beforeSend:beforeSend(),
							success: function(json) {
								if(json.message == "成功") {
