...
layui.use(['element', 'form', 'paging', 'laydate'], function() {
	...
	var showChoosePrize = function(pageNo,name){
		...
		else var url = "/admin/active/getCouponList.do?pageNo="+pageNo+"&pageSize=10";
	...
	$(".save").on("click",function(){
		...
		console.log(data,prizeChooseList)
//		return;
		$.ajax({
			type: "post",
			dataType: "json",
			url: "/admin/active/addActive.do",
			data:data,
			beforeSend:beforeSend(),
			success: function(result) {
				if(result.message == "成功"){
					layer.msg('设置成功！', {
						...
						window.location.href = "/operation/actManagement/activeSucc"
					});
				}else{
	...
	var start = {
		min: laydate.now('YYYY-MM-DD hh:mm'),
//		max: '',
		istime: true,
		istoday: false, 
		format: 'YYYY-MM-DD hh:mm',
