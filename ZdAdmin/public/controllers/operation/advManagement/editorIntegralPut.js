...
$('#playAward').blur(function (){
	var reg = /^[0-9][0-9]{0,8}$/;
	var playAward = $('#playAward').val() || 0;
	if(reg.test(playAward)){
...
$('#integralPutSave').on('click', function (){
	if(clickVerifys()){//验证是CPC&CPM设置 还是 CPM视频设置
		for(var i=0;i<$('.item-adMes .layui-tooltip').length;i++){
			if(!$('.item-adMes .layui-tooltip').eq(i).hasClass('hide')){
				return false;
			}
		}
...
$('#integralSortPre').on('click', function (){
	$('#editorIntegral').removeClass('hide');
...
$('.sortChannelVal').blur(function (){
	...
	if(channelTrue && $sortChannelVal){
		
		if(reg.test($sortChannelVal) && Number($sortChannelVal) > 0 && Number($sortChannelVal) < 9){
			
			$('.tooltip-sortChannel').addClass('hide');
		}else{
			$('.tooltip-sortChannel').removeClass('hide');
		}
	}else{
		$('.tooltip-sortChannel').removeClass('hide');
	}
