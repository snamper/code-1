...
layui.use(['element', 'form', 'layedit', 'laydate', 'upload'], function() {
	...
	var curGoodsImg = "",		//单个点击商品列表图
		...
		merchantId = "",		//商户id
		changeCode = "";
	var pattern = new RegExp("[`~!@#$^&*()=|{}':;'\\[\\].<>/?~！@#￥%……&*（）——|{}【】‘；：”“'。、？]") ;	//特殊字符，不包含“，,”
	var goodsCateList = [],
		goodsCateList1 = [];
	//下一页
	...
	$("#uploadLogoShow").click(function(){	//商品列表 图片替换    
	...
	$(".goodsDetail").change(function(){
		...
		var fs = new FormData();
		if(curGoodsImg){	//替换单张，将原图片删除
			fs.append("oldPath",curGoodsImg);
		}
	...
	$(document).on("click",".delGoodsImg",function(){
		...
		$.ajax({
			type: "post",
			dataType: "json",
			url: "/admin/file/image/delete.do",
			data: {
				imagePath:goodsDetailImgList[index].imageUrl
			},
	...
	form.on('radio(goods_type)', function(data){
		...
		if( productAdAttr == 3) $(".showGoodsDetailImg").hide()
		else $(".showGoodsDetailImg").show()
	});
	//是否允许高风险用户购买
	var risk = 1;
...
//		}
		...
		if(goodsId){		//表示修改
					...
					if(json.message == "成功") {
						layer.msg('保存成功！', {
							...
							if(productAdAttr == 1){		//购买商品
								...
								if(productAdAttr == 2){			//广告商品
									
									$(".goodsTypeHide").hide()
									$(".normalGoods").show().parent().find(".recharge").hide()
								} 								//充值商品
								else if(productAdAttr == 3) $(".normalGoods").hide().parent().find(".recharge").show()
							}
						});
					}else{
			...
			$.ajax({
				...
				dataType: "json",
				contentType: "application/json",
				url: "/admin/product/info/create.do",
				data: JSON.stringify(data),
				beforeSend:beforeSend(),
				success: function(json) {
					if(json.message == "成功") {
	...
	var saveSecMessage = function() {
		var totalPrice = $(".totalPrice").val();	//总价
		var retailPrice = $(".retailPrice").val()	//零售价
		var costPrice = $(".costPrice").val();	//成本价
		if(!$(".costPrice").check().number(0,9) || !$(".costPrice").check().isNum() || !costPrice){
			$(".layui-tooltip-totalPrice").show()
			return;
		}
		if(!$(".retailPrice").check().number(0,9) || !$(".retailPrice").check().isNum()){
	...
	var saveSecondMessage = function() {
		...
		var url = "";
		if(!ifSecFlag){			//新增
			url = '/admin/product/info/set/stock.do'
		}else{					//编辑
			url = '/admin/product/info/set/stock/edit.do';
		}
	 	$.ajax({
			url:url, //上传接口	
			type:"post",
			...
			success: function(json) {
				...
				if(json.message = '成功'){
					...
					for(var i = 0; i < goodsCateList.length; i++){
						if(goodsCateList[i].sortType == 1){
							html += '<option value='+ goodsCateList[i].id +'>'+ goodsCateList[i].sortName +'</option>'
						}
						if(goodsCateList[i].sortType == 2){
							goodsCateList1.push(goodsCateList[i])
						}
					}
...
//		$(".goode_view_img img").attr("src",$("#uploadLogoShow").attr("data-url"))	//商品列表图
	...
	//获取图片数组
	var getImgList = function(role) {
		var dom = $("#firstSrc").parent("li").siblings("li").find("img");
		var imgStr = '';
		if(role == 'new') { //新建商品
			for(var i = 0; i < dom.length; i++) {
				//imgArr.push(dom.eq(i).attr("data-host")+dom.eq(i).attr("src"));	
				imgStr += dom.eq(i).attr("data-host") + dom.eq(i).attr("src") + ',';
			}
		} else { //编辑商品
			for(var i = 0; i < dom.length; i++) {
				imgStr += dom.eq(i).attr("src") + ',';
			}
		}
		if(imgStr != '') {
...
//	})
	...
	$('.costPrice').blur(function() { //商品成本价
		if(!$(".costPrice").check().number(1,9) || !$(".costPrice").check().isNum()){
	...
	$('.productNum').blur(function() { //验证商品数量
		if(!$(".productNum").check().number(1,9) || !$(".productNum").check().isNum()){
			$('.tooltip-productNum').show();
		}else{
			$('.tooltip-productNum').hide();
		}
		
	});
	$('#LAY_demorange_s').blur(function() { //验证有效期
		var $indate = $('#LAY_demorange_s').val();
		if($('#LAY_demorange_s').check().date("DATETIME.FULL")) {	
			$('.layui-time').hide();
		} else {
			$('.layui-time').show();
	...
	$('.rechargeValue').blur(function() { //验证话费充值说明
		if(!$(".rechargeValue").check().number(1,300)) {
			$('.tooltip-rechargeValue').show();
		} else {
	...
	$('.flowRate').blur(function() { //验证流量充值说明
		if(!$(".flowRate").check().number(1,300)) {
			$('.tooltip-flowRate').show();
		} else {
